...
def hamiltonian(psi, psi_past, phi_N, t, paths):
    ...
    for idx in range(N):
        ...
        # ER=EPR update: Link wormhole to entanglement phase
        Delta_phi_ent = np.angle(psi[i,j,k,l,m,n]) - np.angle(psi_past[i,j,k,l,m,n])
        H_worm[idx] += kappa_worm * np.exp(1j * Delta_phi_ent)  # Dual to EPR entanglement
